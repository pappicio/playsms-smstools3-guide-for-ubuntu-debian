...
switch (_OP_) {
	case "user_outgoing":
		...
		if ($queue_code = trim($_REQUEST['queue_code'])) {
			...
			$list = dba_search($table . ' AS A', 'A.smslog_id, A.p_dst, A.p_sms_type, A.p_msg, A.p_footer, A.p_datetime, A.p_update, A.p_status, A.parent_uid, A.queue_code', $conditions, $keywords, $extras, $join);
		} else {
			$search = themes_search($search_category, $base_url);
			$conditions = array(
				'A.parent_uid' => user_getparentbyuid($user_config["uid"]),
			...
			$list = dba_search($table . ' AS A', 'A.smslog_id, A.p_dst, A.p_sms_type, A.p_msg, A.p_footer, A.p_datetime, A.p_update, A.p_status, A.parent_uid, A.queue_code, COUNT(*) AS queue_count', $conditions, $keywords, $extras, $join);
		}
		
		$content = _dialog() . "
			<h2>" . _('My GROUP sent messages') . "</h2>
			" . $queue_label . "
			<p>" . $search['form'] . "</p>
